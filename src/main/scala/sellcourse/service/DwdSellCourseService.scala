...
object DwdSellCourseService {
  ...
  def importSaleCourseLog(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/zhang/ods/salecourse.log")
      .filter(item => {
  ...
  def importCoursePay(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/zhang/ods/coursepay.log")
      .filter(item => {
        val obj = ParseJsonData.getJsonData(item)
        obj.isInstanceOf[JSONObject]
  ...
  def importCoursePay2(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/zhang/ods/coursepay.log").filter(item => {
      val obj: JSONObject = ParseJsonData.getJsonData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partition => {
      partition.map(item => {
        val js: JSONObject = ParseJsonData.getJsonData(item)
        val orderid: String = js.getString("orderid")
        val paymoney: String = js.getString("paymoney")
        val discount: String = js.getString("discount")
        val createtime: String = js.getString("createitme")
  ...
  def importCourseShoppingCart(ssc: SparkContext, sparkSession: SparkSession) = {
    import sparkSession.implicits._
    ssc.textFile("/user/zhang/ods/courseshoppingcart.log")
      .filter(item => {
        val obj = ParseJsonData.getJsonData(item)
        obj.isInstanceOf[JSONObject]
      }).mapPartitions(partitions => {
